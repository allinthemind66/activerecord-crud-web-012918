...
def can_get_the_first_item_in_the_database
  item = Movie.first.title
end

def can_get_the_last_item_in_the_database
  item = Movie.last.title
end

def can_get_size_of_the_database
  item = Movie.all.length
end

def can_find_the_first_item_from_the_database_using_id
  item = Movie.find_by_id(1).title
end
...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  item = Movie.find_by(title: "Title")
end
...
def can_destroy_a_single_item
  ...
  Movie.where(title: "That One Where the Guy Kicks Another Guy Once").delete
end
